...
export function executeDepProfTask(discopopView: DiscoPoPViewProvider, ifiles? : string[]){
        ...
        files.forEach(async (file, index) => {
            ...
            exec(`${vscode.workspace.getConfiguration("discopopvsc").get("clang")} -g -O0 -S -emit-llvm -fno-discard-value-names -Xclang -load -Xclang ${discopopView.buildPath}/libi/LLVMDPInstrumentation.so -mllvm -fm-path -mllvm ./FileMapping.txt -o ${outFileName}_dp.ll ${file}`, { cwd: discopopView.folderPath }, (error, stdout, stderr) => {
                if (error) {
                    vscode.window.showErrorMessage(`Error during step 1 of identifying data dependencies in file ${outFileName}`);
                    console.log(`error: ${error.message}`);
                    return;
                }
                exec(`${vscode.workspace.getConfiguration("discopopvsc").get("clang")} ${outFileName}_dp.ll -o ${outFileName}_dp_run -L${discopopView.buildPath}/rtlib -lDiscoPoP_RT -lpthread`, { cwd: discopopView.folderPath }, (error, stdout, stderr) => {
                    if (error) {
                    ...
                    exec(`./${outFileName}_dp_run`, { cwd: discopopView.folderPath }, (error, stdout, stderr) => {
                        if (error) {
